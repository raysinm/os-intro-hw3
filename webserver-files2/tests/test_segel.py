...
def test_basic(policy, server_port):
    ...
    with Server("./server", server_port, 1, 1, policy, False) as server:
        sleep(0.1)
...
def test_pool(policy, threads, queue_size, server_port):
    ...
    with Server("./server", server_port, threads, queue_size, policy, False) as server:
        sleep(0.1)
...
def test_drop_head(threads, num_clients, queue_size, times, server_port):
    with Server("./server", server_port, threads, queue_size, "dh", False) as server:
        sleep(0.1)
        for _ in range(times):
            slowers = []
            dropped = []
...
def test_drop_tail(threads, num_clients, queue_size, times, server_port):
    with Server("./server", server_port, threads, queue_size, "dt", False) as server:
        sleep(0.1)
        for _ in range(times):
            slowers = []
            alive = []
...
def test_drop_random(threads, num_clients, queue_size, times, server_port):
    with Server("./server", server_port, threads, queue_size, "random", False) as server:
        sleep(0.1)
        for _ in range(times):
            slowers = []
            others = []
