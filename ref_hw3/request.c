...
void requestError(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg, struct timeval arrival,
                  struct timeval handle_time, int *static_thread, int *dynamic_thread, int *thread_total,
                  int index_thread) {
...
void requestHandle(int fd, struct timeval arrival, struct timeval handle_time, int *static_thread, int *dynamic_thread,
    ...
    if (strcasecmp(method, "GET")) {
        ...
        requestError(fd, method, "501", "Not Implemented", "OS-HW3 Server does not implement this method", arrival,
                     handle_time, static_thread, dynamic_thread, thread_total ,index_thread);
        return;
    }
    requestReadhdrs(&rio);
    ...
    if (stat(filename, &sbuf) < 0) {
        thread_total[index_thread]++;
        requestError(fd, filename, "404", "Not found", "OS-HW3 Server could not find this file", arrival, handle_time,
                     static_thread, dynamic_thread, thread_total, index_thread);
        return;
    }
    ...
    if (is_static) {
        if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
            thread_total[index_thread]++;
            requestError(fd, filename, "403", "Forbidden", "OS-HW3 Server could not read this file", arrival,
                         handle_time, static_thread, dynamic_thread, thread_total, index_thread);
            return;
        }
        thread_total[index_thread]++;
